...
router.set('/route', { POST: api.evaluateTarget })
router.set('/api/targets', { GET: api.getAllTargets, POST: api.saveTarget })
router.set('/api/target/:id', { GET: api.getTarget, POST: api.updateTarget })

/**
 * Reset Existing Targets at midnight UTC
 */

cron.schedule('0 0 * * *', async () => {
  await api.resetTargetCount()
  console.log('New day, maximum targets per day have been reset for all target ids.')
})
